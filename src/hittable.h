...
bool translate::hit(const ray& r, double t_min, double t_max, hit_record& rec) const {
	ray r_move = ray(r.origin() - offset, r.direction());
	if (!object->hit(r_move, t_min, t_max, rec))
	{
		return false;
	}
	rec.p = rec.p + offset;
	rec.set_face_normal(r_move, rec.normal);
	return true;
}

bool translate::boundingbox(aabb& out) const {

	if (!object->boundingbox(out))
	{
		return false;
	}
	out = aabb(out.min() + offset, out.max() + offset);
	return true;
}
